...
# --- 論文との意図的な逸脱: 磁場の下部境界条件 -------------------------------
# Rempel (2006) §2.2 は "B_Phi vanishes at both radial boundaries" と書いており、
# その分岐は boundary_condition.py の 'R06' として実装してある。
# **しかし本実装は既定の 'vertical' (下部 d(rB_Phi)/dr = 0、対称) を使う。**
#
# 理由 1 (物理): r = 0.65 RSUN は放射層の内部で、実際の太陽には境界がない。
#   放射層は良導体なので磁場は蓄えられるべきで、B_Phi = 0 は境界を
#   トロイダル磁束と磁気エネルギーの吸い込みにしてしまう。対称条件なら
#   拡散フラックスがゼロで、A = 0 と合わせて閉じた境界になる。
#   **論文自身が「B_Phi だけは closed boundary condition ではない」と
#   §3.2 で認めており、上部境界の抵抗フラックスが無視できるとしか
#   言っていない (下部については何も言っていない)。**
#   式 (22) の収支が Q_Lambda 比 1e-5 - 1e-4 で閉じるのは対称境界だから。
#
# 理由 2 (数値): B_Phi = 0 が作る抵抗層 sqrt(eta_c * t) は 1 年で 0.00255 R。
#   108x72 の底のセル幅は 0.00386 R で**入らない**。実測でも 108x72 の
#   'R06' は 4.4 年目から 1.1 年で倍増する局所的な指数成長を起こし、
#   13 年で max|B_phi| = 92 T に達して発散した (0.735R 赤道の値は正常)。
#
# 詳細: doc/dev_records/2026-08-23_paper_audit2.md
