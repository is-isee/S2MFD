...
def defunction_simulation(cfg=None, parameter_file=None, datadir=None, startpoint=0, endpoint=0):
   ...
   with ProcessPoolExecutor() as executor:
      ...
      for A_sample, omg_sample, B_sample, C_sample in product(A, Omg, B, C):
         futures.append(executor.submit(run_defunction, parameter_file, A_sample, omg_sample, B_sample, C_sample, datadir, n1, Bpht, Apht, uu0t, so0t, nt, ndt, timet, startpoint, endpoint, Sunspot_N))
...
def run_defunction(parameter_file, A_sample, omg_sample, B_sample, C_sample, datadir, n1, Bpht, Apht, uu0t, so0t, nt, ndt, timet, startpoint, endpoint, Sunspot_N):
   cfg = S2MFD.Cfg(parameter_file)
   sim = S2MFD.Simulation(cfg)
