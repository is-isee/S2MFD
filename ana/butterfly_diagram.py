...
c2 = ax2.pcolormesh(time_year,grid.th/np.pi*180,B_0*Brrt_s[:,ns:ne],vmax=B_range*1e3,vmin=-B_range*1e3,cmap='bwr',shading='auto')
...
mask_p = Bpht_lim > 3.0
mask_m = Bpht_lim < -3.0
ax1.contourf(time_year, grid.th/np.pi*180, mask_p, levels=[0.5, 1.5], colors=['black'])
ax1.contourf(time_year, grid.th/np.pi*180, mask_m, levels=[0.5, 1.5], colors=['black'])
