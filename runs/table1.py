...
def detrend(t, x, win=None):
    r"""区間内の 1 次トレンドを引いて振動成分だけ残す。

    表 1 の量は :math:`\max(\Omega-\bar\Omega)` で、:math:`\bar\Omega`
    は**時間平均**である。素直には平均を引けばよいが、解がまだ緩和して
    いる間は永年ドリフトがその上に乗るので 1 次まで落とす。

    **移動平均を使ってはいけない (2026-08-25 に判明)。**
    以前は幅 18 年 (= 1 サイクル) の移動平均を引いて両端を ``n//2`` ずつ
    捨てていた。ところが解析区間も 20 年なので、

        末尾 20 年 = 704 点、窓 18 年 = 632 点、両端 316 点ずつ捨てる
        -> **残るのは 72 点 = 2.0 年ぶん**

    となり、「末尾 20 年の最大」のつもりが「どこか 2 年間の最大」に
    なっていた。これで振幅が 2 割ふらつき、a250 と a500 が論文比
    0.67-0.70 に見えていた。1 次で引くと 1.02 / 0.95 で合う。

    **偏りが 1 割ある。** 解析区間 20 年に対し周期が 18 年なので 1.1
    サイクルしか入らず、1 次の当てはめが振動そのものを一部吸う。
    振幅 3.0 の純正弦 + 直線に対して 3.28 を返す (+9 パーセント)。
    全ランに同じだけ乗るのでラン間の比較には効かないが、論文値との比を
    1 割の精度で議論してはいけない。

    ``win`` は後方互換のために残してあるだけで使わない。
    """
    p = np.polyfit(t, x, 1)
