...
# butterfly diagram
B_0 = 1.e5
c1 = ax1.pcolormesh(time_y, grid.th/np.pi*180, B_0*Bpht_c[:,ns:ne], cmap='bwr', shading='auto')
c2 = ax2.pcolormesh(time_y,grid.th/np.pi*180,B_0*Brrt_s[:,ns:ne],vmax=B_range*1e3,vmin=-B_range*1e3,cmap='bwr',shading='auto')
"""
# 描画の範囲指定なし
...
mask_p = Bpht_lim > 3.0
mask_m = Bpht_lim < -3.0
ax1.contourf(time_y, grid.th/np.pi*180, mask_p, levels=[0.5, 1.5], colors=['black'])
ax1.contourf(time_y, grid.th/np.pi*180, mask_m, levels=[0.5, 1.5], colors=['black'])
