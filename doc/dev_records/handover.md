...
- **フルスケール検証 (2026-08-19 実施)** — 詳細は [2026-08-19_fullscale_results.md](2026-08-19_fullscale_results.md):
  - **観測データ 1944–1996: 相関 r = 0.871 (Stage1) / 0.900 (Stage2)。論文表4.4 の r=0.873 とほぼ一致**
  - twin experiment: Step2 で r=0.988, e=0.0120, u0誤差 5.77%, s0誤差 9.81% (論文表4.1: 0.995, 0.00514, 2.87%, 5.25%)
  - boston (Intel) と paris (AMD) で推定パラメタが16桁一致
  - 高速化版と原実装で推定パラメタ11個すべてが15桁一致・世代数も一致
...
### すぐ着手できるもの
1. ~~フルスケール twin experiment~~ / ~~観測データでの実推定~~ → **2026-08-19 実施済み**
   ([結果](2026-08-19_fullscale_results.md))
2. **残り5区間の推定** (1723–1775, 1775–1833, 1833–1889, 1889–1944, 1996–2024)。
   高速化後は全6区間で約1.5時間。ダルトンミニマム区間は `--fitness period` を使う。
3. ~~GitHub Actions での CI~~ → **実施済み** (`.github/workflows/`)
