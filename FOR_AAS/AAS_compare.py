...
# 分析範囲、対象は手で決める
datadir = '../OBS_results_no1/data_obs_full/'
n0 = 210
n1 = 2957
alpha = 0.9  # 評価関数の重み
...
plt.scatter(SN1, SN2, s=4, alpha=0.7, label=f'r = {cc:.2f}')
...
plt.plot(x_line, fit_fn(x_line), color="red", linewidth=2, label="Regression line")

# 軸や凡例の整形
plt.xlabel("Obs. SN", fontsize=12)
plt.ylabel("Theoretical SN", fontsize=12)
# plt.title("相関関係の散布図", fontsize=14)
plt.legend()
...
plt.figure(figsize=(5, 5))
plt.scatter(period_num, period_obs, s=4, alpha=0.7, label=f'r = {period_corr:.2f}')
...
# 軸や凡例の整形
plt.xlabel("Obs. Period", fontsize=12)
plt.ylabel("Theoretical Period", fontsize=12)
# plt.title("相関関係の散布図", fontsize=14)
plt.legend()
plt.grid(True, linestyle="--", alpha=0.6)
...
plt.figure(figsize=(5, 5))
plt.scatter(SN1_max, SN2_max, s=4, alpha=0.7, label=f'r = {SN_corr:.2f}')
...
plt.plot(time2, uu0t2, 'b', label='estimation',linewidth=2.5)
plt.xlabel('Time(years)',fontsize=3*size)
...
plt.plot(time2, so0t2, 'b', label='estimation',linewidth=2.5)
...
axes[0].plot(time2, SN2, 'b', label='estimation', linewidth=2.5)
axes[0].set_ylabel("Sunspot number", fontsize=12*size2)
axes[0].legend(loc="upper right", fontsize=10*size2)
axes[0].tick_params(axis='both', labelsize=10*size2)
axes[0].grid(True, linestyle="--", alpha=0.6)

# --- 中央 ---
axes[1].plot(time2, uu0t2, 'b', label='estimation', linewidth=2.5)
ymax = max(np.max(uu0t2), np.max(uu0t2)) * 1.1
axes[1].set_ylim(bottom=0, top=ymax)
axes[1].set_ylabel(r"$u_0$", fontsize=12*size2)
axes[1].legend(loc="upper right", fontsize=10*size2)
axes[1].tick_params(axis='both', labelsize=10*size2)
axes[1].grid(True, linestyle="--", alpha=0.6)

# --- 一番下 ---
axes[2].plot(time2, so0t2, 'b', label='estimation', linewidth=2.5)
ymax = max(np.max(so0t2), np.max(so0t2)) * 1.1
axes[2].set_ylim(bottom=0, top=ymax)
axes[2].set_ylabel(r"$s_0$", fontsize=12*size2)
axes[2].set_xlabel("Years", fontsize=12*size2)
axes[2].legend(loc="upper right", fontsize=10*size2)
axes[2].tick_params(axis='both', labelsize=10*size2)
